<!-- THERE ARE THREE QUERIES(JOIN, DELETE AND UPDATE) YOU NEED TO FILL FOR THIS FILE. THEY ARE ON LINES 61, 89 and 100 RESPECTIVELY. done-->
	...
	<div class="container allratings">
		<div class="row">
			...
			<div class="col-6">
				...
				$query = "SELECT movies.movie_name, reviews.ratings, reviews.user_id, reviews.movie_id
						FROM movies JOIN reviews ON reviews.movie_id = movies.movie_id 
				...
				if (isset($_POST["delrecord"])) {
					...
					$query = "DELETE FROM reviews WHERE movie_id = $movieid AND user_id = $userid";
				...
				if (isset($_POST["rate"])) {
					...
					$query = "UPDATE reviews SET ratings = $rating WHERE user_id = $userid AND movie_id = $movieid";
					$query_run = mysqli_query($con, $query);
					if ($query_run) {
